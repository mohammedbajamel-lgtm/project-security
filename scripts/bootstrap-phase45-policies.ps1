...
function Get-Resources($Module) {
    ...
    foreach ($child in @($Module.child_modules)) {
        if ($null -ne $child) { Get-Resources $child }
    }
}
...
$targets = @(
    ...
    @{ Address = "module.iam.aws_iam_role_policy.correlation"; Role = "cloudsec-dev-correlation-role"; Name = "cloudsec-correlation-permissions" }
)
...
Write-Host "PASS: Phase 4-5 IAM policies bootstrapped from the exact reviewed plan."
